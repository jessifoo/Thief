...
namespace FarseerPhysics.Common
{
    ...
    public class Vertices : List<FVector2>
    {
        ...
        public bool CheckPolygon(out int error, out string errorMessage)
        {
            ...
            if (Count < 3 || Count > FSSettings.MaxPolygonVertices)
            {
                error = 0;
            }
            if (!IsConvex())
            ...
            for (int i = 0; i < Count; ++i)
            {
                ...
                if (angle <= FSSettings.AngularSlop)
                {
                    error = 4;
                    break;
                }
            ...
            while (foundDupe)
            {
                ...
                for (int i = 0; i < nNodes; ++i)
                {
                    ...
                    for (int j = i + 1; j < nNodes; ++j)
                    {
                        ...
                        if (diff.LengthSquared() <= FSSettings.Epsilon * FSSettings.Epsilon)
                        {
                            if (nActive <= 3)
                                return new Vertices();
        ...
        private class PolyNode
        {
            ...
            public PolyNode GetRightestConnection(PolyNode incoming)
            {
                ...
                Debug.Assert(inLength > FSSettings.Epsilon);
                ...
                for (int i = 0; i < NConnected; ++i)
                {
                    ...
                    testDir.Normalize();
                    Debug.Assert(testLengthSqr >= FSSettings.Epsilon * FSSettings.Epsilon);
