...
namespace FarseerPhysics.Dynamics
{
    ...
    public class FSWorld
    {
        ...
        public FSIsland Island = new FSIsland();
        private FSBody[] _stack = new FSBody[64];
        private bool _stepComplete;
        private HashSet<FSBody> _bodyAddList = new HashSet<FSBody>();
        private HashSet<FSBody> _bodyRemoveList = new HashSet<FSBody>();
        ...
        private FSWorld()
        {
            ...
            BreakableBodyList = new List<FSBreakableBody>();
            BodyList = new List<FSBody>(32);
        ...
        public List<FSBreakableBody> BreakableBodyList { get; private set; }
        ...
        public FSContactManager ContactManager { get; private set; }

        /// <summary>
        /// Get the world body list.
        /// </summary>
        /// <value>Thehead of the world body list.</value>
        public List<FSBody> BodyList { get; private set; }
        ...
        private void ProcessRemovedJoints()
        {
            if (_jointRemoveList.Count > 0)
            {
                foreach (FarseerJoint joint in _jointRemoveList)
                {
                    ...
                    FSBody bodyA = joint.BodyA;
                    FSBody bodyB = joint.BodyB;
        ...
        private void ProcessAddedBodies()
        {
            ...
            {
                foreach (FSBody body in _bodyAddList)
                {
#if USE_AWAKE_BODY_SET
        ...
        private void ProcessRemovedBodies()
        {
            ...
            {
                foreach (FSBody body in _bodyRemoveList)
                {
        ...
        public void Step(float dt)
        {
            ...
            // Handle TOI events.
            if (FSSettings.ContinuousPhysics)
            {
                SolveTOI(ref step);
            }
        ...
        public void QueryAABB(Func<FSFixture, bool> callback, ref AABB aabb)
        {
        ...
        public void RayCast(RayCastCallback callback, FVector2 point1, FVector2 point2)
        {
            ...
            ContactManager.BroadPhase.RayCast((rayCastInput, proxyId) =>
                                                  {
                                                      FixtureProxy proxy = ContactManager.BroadPhase.GetProxy(proxyId);
                                                      FSFixture fixture = proxy.Fixture;
        ...
        private void Solve(ref FSTimeStep step)
        {
            ...
            if (stackSize > _stack.Length)
                _stack = new FSBody[Math.Max(_stack.Length * 2, stackSize)];
        ...
        private void SolveTOI(ref FSTimeStep step)
        {
            Island.Reset(2 * FSSettings.MaxTOIContacts, FSSettings.MaxTOIContacts, 0, ContactManager);
            ...
            for (int i = 0; i < ContactManager.ContactList.Count; i++)
            {
                    ...
                    if (c.TOICount > FSSettings.MaxSubSteps)
                    {
                        continue;
                    }
                    ...
                    {
                        FSFixture fA = c.FixtureA;
                        FSFixture fB = c.FixtureB;
                ...
                if (minContact == null || 1.0f - 10.0f * FSSettings.Epsilon < minAlpha)
                {
                ...
                FSFixture fA1 = minContact.FixtureA;
                FSFixture fB1 = minContact.FixtureB;
                FSBody bA0 = fA1.Body;
                FSBody bB0 = fB1.Body;
        ...
        public FSFixture TestPoint(FVector2 point)
        {
            AABB aabb;
            FVector2 d = new FVector2(FSSettings.Epsilon, FSSettings.Epsilon);
            aabb.LowerBound = point - d;
            aabb.UpperBound = point + d;

            FSFixture myFixture = null;
        ...
        /// <returns></returns>
        public List<FSFixture> TestPointAll(FVector2 point)
        {
            AABB aabb;
            FVector2 d = new FVector2(FSSettings.Epsilon, FSSettings.Epsilon);
            aabb.LowerBound = point - d;
