...
public class RageCurve
{
	...
	// TODO: Have to find the distance considering a given RageSplinePoint[]
	private float CalculateReferenceDistance(Vector3 referencePoint, float referenceOffset) {
		return Vector3.SqrMagnitude(referencePoint - GetPoint(referenceOffset));
	}

	private float GetOffsetFromIndex(int idxPrev) {
		return GetNearestSplinePoint(points[idxPrev].point, 1000);
	}
